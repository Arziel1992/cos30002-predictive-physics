...
export class BallisticsSim {
	...
	update(params, canvasSize) {
		...
		this.target.y =
			this.target.baseY + Math.sin(this.target.time * targetSpeed) * 0.3;
		this.target.velocity.y =
			Math.cos(this.target.time * targetSpeed) * 0.3 * targetSpeed;
		...
		for (let i = this.projectiles.length - 1; i >= 0; i--) {
			...
			if (p.type === "projectile") {
				...
				const dist = Math.sqrt((p.x - targetPxX) ** 2 + (p.y - targetPxY) ** 2);
				if (dist < 30) {
					this.stats.hits++;
				...
				if (p.life === 9) {
					const distToLine = this.pointLineDist(
						targetPxX,
						targetPxY,
						p.x1,
						p.y1,
